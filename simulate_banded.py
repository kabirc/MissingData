...
if __name__ == '__main__':
    ...
    # Run a simulation
    num_trials = 50 
    alpha_list = np.linspace(0.5, 0.99, 20) # missingness parameter
    ...
    for alpha in alpha_list:
        ...
        for i in range(num_trials):
            banded_dict_approx = {'alpha': alpha, 'approx': True, 'n': n, 'p': p}
            banded_dict = {'alpha': alpha, 'approx': False, 'n': n, 'p': p}
    ...
    print(data.shape)
    np.savetxt('BandedGaussian.dat', data, fmt=['%.2f', '%.4f', '%.6f', '%.8f',\
            '%.10f', '%.12f', '%.14f'],\
